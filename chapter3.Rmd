# Chapter 3 - data wrangling and analysis of the Portuguese student performance dataset
...
## Data wrangling part
## Reading, combining and checking structure of the data  
These datasets consists of data from two questionnaires from Portuguese schools.  
The first dataset contains answers from the subject of mathematics and the other from Portuguese language.  
...
# Writing the new combined data as csv file.
...
```{r}
# Reading the data back into R.
alc <- read.csv("data/alc.csv")

print(str(alc))
print(colnames(alc))
```
The variables found in these datasets contain information about the students, their families, hobbies etc. totalling in 35 different variables for each of the 370 students.

### Hypothesis about the data and the relationships between selected 4 variables and amount of drinking  
I found the differences between men and women so interesting that I decided to keep them in the following comparison plots. I selected the sex, family relation, going out and mothers education (and out of curiosity the fathers education).  
My hypothesis for these are as follows:  
1. On average, men drink more and those that do score worse in the final grades (G3).  
2. Higher quality of family relation helps prevent higher alcohol usage.  
3. People who go out more have more likely high alcohol usage.  
4. A high education level for the mother lowers the likelyhood of high alcohol usage.  
...
### Coefficients and their meaning on the hypothesis
...
**1 (SexM)** - Yes, this was statistifically significant (P-score of 8.89e-05).  
**2 (famrel)** - Yes, also significant (P-score of 0.00166).  
**3 (goout)** - Yes, also significant (P-score of 6.06e-11).  
**4 (Medu)** - Not significant (P-score of 0.35346), neither is the fathers level of education (P-score of 0.68414).  
...
Going out more results in ~2.23 times more likely to having high consumption.  
I think the earlier hypothesis stands, but the mothers education (Medu) does have an lowering effect on the high consumption.
```{r}
...
```
**On average, we will get ~22 % of the predictions wrong in the training data. The model seems to do well when predicting false, but seems to underestimate the true predictions.**
